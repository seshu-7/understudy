...
corevantage-servicing (web), tenant `northstar`
Entry point: http://127.0.0.1:4502/servicing/login.asp
...
4. **enter the member number into the search field**
   ...
   - risk: `safe`
   - checkpoint: screen shows button named "Find Member" within "Servicing > Member Search" (frame: mainfrm)
5. **Click the Search button to initiate the search for member 100234**
   - action: click button named "Find Member" within "Servicing > Member Search" (frame: mainfrm)
   - risk: `safe`
...
- human-edited since compilation: yes

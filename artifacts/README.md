...
capability reused against a second real tenant, not re-recorded: produced by
`npm run overlay` (`src/artifact/overlay.ts`) from the file above, with the
member-search button's name rewritten from "Search" (meridian) to "Find
Member" (northstar) and the target binding pointed at northstar's own server.
REPORT.md §4 has the full write-up, including what actually happens to the
*un*-overlaid capability against the same tenant, checked rather than
assumed.
...
program hash identically, and two that don't, don't - a real check, not an
